...
router.post("/buildings", requireAuth, requireRole("admin"), async (req, res) => {
  ...
  try {
    ...
    await recordAudit({ entityType: "building", entityId: b.id, action: "create", newData: b, userId: req.authUser!.id });
...
router.patch("/buildings/:id", requireAuth, requireRole("admin"), async (req, res) => {
  const id = parseInt(req.params.id as string);
  const { nameAr, code, addressAr } = req.body;
  try {
    ...
    await recordAudit({ entityType: "building", entityId: id, action: "update", oldData: old, newData: b, userId: req.authUser!.id });
    res.json({ id: b.id, nameAr: b.nameAr, code: b.code, addressAr: b.addressAr ?? null, archived: b.archived, createdAt: b.createdAt });
  } catch (err) {
    logger.error(err);
    res.status(500).json({ error: "Server error" });
...
router.delete("/buildings/:id", requireAuth, requireRole("admin"), async (req, res) => {
  const id = parseInt(req.params.id as string);
  try {
    const [old] = await db.select().from(buildingsTable).where(eq(buildingsTable.id, id)).limit(1);
    if (!old) { res.status(404).json({ error: "Not found" }); return; }
    await db.update(buildingsTable).set({ archived: true }).where(eq(buildingsTable.id, id));
    await recordAudit({ entityType: "building", entityId: id, action: "archive", oldData: old, userId: req.authUser!.id });
    res.json({ ok: true });
  } catch (err) {
    logger.error(err);
    res.status(500).json({ error: "Server error" });
